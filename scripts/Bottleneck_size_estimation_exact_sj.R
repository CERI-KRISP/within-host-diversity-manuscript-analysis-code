...
parser$add_argument("--Nb_max", type="integer", default= 1000,
...
#if (args$file == "no_input_return_error" ) { stop("file with lists of donor and recipient frequencies is a required argument.", call.=FALSE)} ##calling from script so handling this from the calling script
...
#donor_and_recip_freqs_observed <- read.table(freqs_file) # table of SNP frequencies in donor and recipient
...
donor_freqs_recip_freqs_and_reads_observed <- read.table(freqs_file)  #table of SNP frequencies and reads in donor and recipient
...
message("New row count: ", new_row_count)
if(new_row_count > 0)
{
...
}else{ Max_LL_bottleneck = 0
      lower_CI_bottleneck = 0
      upper_CI_bottleneck = 0
